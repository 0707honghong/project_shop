    ...
    <div id="menu-list">
        <ul id="menu-ul">
            <?php if(is_array($menus)): foreach($menus as $key=>$menu): if(($menu["parent_id"]) == "0"): ?><li class="explode" key="02_cat_and_goods" name="menu">
                        <?php echo ($menu["name"]); ?>
                        <ul>
                            <?php if(is_array($menus)): foreach($menus as $key=>$second): if(($second['parent_id']) == $menu['id']): ?><li class="menu-item">
                                        <a href="<?php echo U($second['path']);?>" target="main-frame"><?php echo ($second["name"]); ?></a>
